...
if valid_results.empty:
    ...
    plt.title('Temps d\'exécution en fonction du nombre de threads readers-writers')
    plt.xlabel('Nombre de threads')
    plt.ylabel('Temps d\'exécution (secondes)')
    plt.xticks(valid_results['Nb_threads'])
    plt.yticks(np.arange(0, max(valid_results['Mean_time'])+0.02, 0.009))  
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
